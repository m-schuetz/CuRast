import os
import shutil
import argparse
import torch
from skimage.metrics import peak_signal_noise_ratio, structural_similarity
import pillow_jxl
import pillow_avif
from PIL import Image, ImageTk, features
import numpy as np
import flip_evaluator as flip
import lpips
from pathlib import Path
import subprocess
import matplotlib.pyplot as plt
from matplotlib.patches import Polygon
from matplotlib.ticker import MultipleLocator
from collections import namedtuple
import csv

loss_fn_alex = lpips.LPIPS(net='alex')


# check what jpeg encoder PIL uses
# print("PIL modules: ")
# for item in features.get_supported_modules():
# 	version = features.version_module(item)
# 	print(f"- {item}, {version}")
# print("PIL codecs: ")
# for item in features.get_supported_codecs():
# 	version = features.version_codec(item)
# 	print(f"- {item}, {version}")
# print("PIL features: ")
# for item in features.get_supported_features():
# 	version = features.version_feature(item)
# 	print(f"- {item}, {version}")

#def round_to_multiple(n, multiple):
#	return multiple * round(n / multiple)

# def computePlotStepsize(valueRange, digits):
# 	#stepsize = round(100 * valueRange / 8) / 100
# 	round(valueRange / 8, digits)
# 	return multiple * round(n / multiple)

def load_image(path):
	"""
	Load an image from the given path using PIL.
	Returns an RGB numpy array.
	"""
	try:
		img = Image.open(path).convert("RGB")
		return np.array(img)
	except Exception as e:
		print(f"Error loading image {path}: {e}")
		return None

parser = argparse.ArgumentParser(description="Example script with -o option")
parser.add_argument('-o', '--output', type=str, help='output directory')
parser.add_argument('-i', nargs='+', help='list of folders created with convert_images.py')

# Parse the arguments
args = parser.parse_args()

outputDirectory = args.output

def runFlip(img_reference, img_test):
	img_reference_norm = img_reference.astype(np.float32) / 255.0
	img_test_norm = img_test.astype(np.float32) / 255.0
	flipErrorMap, meanFLIPError, parameters = flip.evaluate(img_reference_norm, img_test_norm, "LDR")

	return meanFLIPError

def runLpips(img_reference, img_test):
	ref_tensor = torch.from_numpy(img_reference).permute(2, 0, 1).unsqueeze(0).float()
	ref_tensor = (ref_tensor / 127.5) - 1.0

	test_tensor = torch.from_numpy(img_test).permute(2, 0, 1).unsqueeze(0).float()
	test_tensor = (test_tensor / 127.5) - 1.0

	d = loss_fn_alex(ref_tensor, test_tensor)

	return d.item()

def compareImages():

	Record = namedtuple("Record", ["algorithm", "label", "bpp", "psnr", "ssim", "flip", "lpips"])

	for sourcePath in args.i:

		# Create list of converter image files, grouped by algorithm
		listByAlgorithm = {
			"astc": [],
			"bc": [],
			"jpegturbo": [],
			"jpegli": [],
			"jpegXL": [],
			"AVIF": [],
		}

		files = os.listdir(sourcePath)
		for file in files:
			for key in listByAlgorithm:
				if(file.startswith(key) and file.endswith(("png", "avif", "jpg", "jxl"))):
					listByAlgorithm[key].append(file)

		print(listByAlgorithm)

		filename = Path(sourcePath).name
		img_reference = load_image(f"{sourcePath}/{filename}")
		records = []

		# ASTC
		astcFormats = [str.replace(".png", "") for str in listByAlgorithm["astc"]]
		astcFormats = sorted(astcFormats, key=lambda x: (len(x), x))
		for format in astcFormats:
			print(format)
			filesize = os.path.getsize(f"{sourcePath}/{format}.dds")
			image = load_image(f"{sourcePath}/{format}.png")
			width, height, channels = image.shape

			bytesPerPoint = filesize / (width * height)
			bpp = bytesPerPoint * 8
			psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
			ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
			flip_value = runFlip(img_reference, image)
			lpips_value = runLpips(img_reference, image)

			print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

			label = format.replace("astc_ldr_", "")
			records.append(Record("ASTC", label, bpp, psnr_value, ssim_value, flip_value, lpips_value))

		# BC
		for file in listByAlgorithm["bc"]:
			format = file.replace(".png", "")
			filesize = os.path.getsize(f"{sourcePath}/{format}.dds")
			image = load_image(f"{sourcePath}/{format}.png")
			width, height, channels = image.shape

			bytesPerPoint = filesize / (width * height)
			bpp = bytesPerPoint * 8
			psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
			ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
			flip_value = runFlip(img_reference, image)
			lpips_value = runLpips(img_reference, image)

			print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

			records.append(Record("BC", format, bpp, psnr_value, ssim_value, flip_value, lpips_value))

		# JPEG 
		for file in listByAlgorithm["jpegturbo"]:
			compressedPath = f"{sourcePath}/{file}"
			filesize = os.path.getsize(compressedPath)
			image = load_image(compressedPath)
			width, height, channels = image.shape
			quality = int(file.replace(".jpg", "").replace("jpegturbo_", ""))

			# if (quality < 50): continue

			bytesPerPoint = filesize / (width * height)
			bpp = bytesPerPoint * 8
			psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
			ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
			flip_value = runFlip(img_reference, image)
			lpips_value = runLpips(img_reference, image)

			format = f"JPEG {quality}%"
			print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

			label = f"{quality}"
			records.append(Record("JPEG (jpegturbo)", label, bpp, psnr_value, ssim_value, flip_value, lpips_value))

		# JPEGLI
		for file in listByAlgorithm["jpegli"]:
			compressedPath = f"{sourcePath}/{file}"
			filesize = os.path.getsize(compressedPath)
			image = load_image(compressedPath)
			width, height, channels = image.shape
			quality = int(file.replace(".jpg", "").replace("jpegli_", ""))

			# if (quality < 50): continue

			bytesPerPoint = filesize / (width * height)
			bpp = bytesPerPoint * 8
			psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
			ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
			flip_value = runFlip(img_reference, image)
			lpips_value = runLpips(img_reference, image)

			format = f"JPEGLI {quality}%"
			print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

			label = f"{quality}"
			records.append(Record("JPEGLI", label, bpp, psnr_value, ssim_value, flip_value, lpips_value))

		# JPEG XL
		for file in listByAlgorithm["jpegXL"]:
			compressedPath = f"{sourcePath}/{file}"
			filesize = os.path.getsize(compressedPath)
			image = load_image(compressedPath)
			width, height, channels = image.shape
			quality = int(file.replace(".jxl", "").replace("jpegXL_", ""))

			# if (quality < 50): continue

			bytesPerPoint = filesize / (width * height)
			bpp = bytesPerPoint * 8
			psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
			ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
			flip_value = runFlip(img_reference, image)
			lpips_value = runLpips(img_reference, image)

			format = f"JPEG XL {quality}%"
			print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

			label = f"{quality}"
			records.append(Record("JPEG XL", label, bpp, psnr_value, ssim_value, flip_value, lpips_value))

		# AVIF
		for file in listByAlgorithm["AVIF"]:
			compressedPath = f"{sourcePath}/{file}"
			filesize = os.path.getsize(compressedPath)
			image = load_image(compressedPath)
			width, height, channels = image.shape
			quality = int(file.replace(".avif", "").replace("AVIF_", ""))

			# if (quality < 50): continue

			bytesPerPoint = filesize / (width * height)
			bpp = bytesPerPoint * 8
			psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
			ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
			flip_value = runFlip(img_reference, image)
			lpips_value = runLpips(img_reference, image)

			format = f"AVIF {quality}%"
			print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

			label = f"{quality}"
			records.append(Record("AVIF", label, bpp, psnr_value, ssim_value, flip_value, lpips_value))

		# # HEIC
		# for quality in qualityLevels:
		# 	compressedPath = f"{outputDirectory}/{filename}/PIL_HEIC_{quality}.heic"
		# 	filesize = os.path.getsize(compressedPath)
		# 	image = load_image(compressedPath)
		# 	width, height, channels = image.shape

		# 	bytesPerPoint = filesize / (width * height)
		# 	bpp = bytesPerPoint * 8
		# 	psnr_value = peak_signal_noise_ratio(img_reference, image, data_range=255)
		# 	ssim_value = structural_similarity(img_reference, image, data_range=255, channel_axis=2)
		# 	flip_value = runFlip(img_reference, image)
		# 	lpips_value = runLpips(img_reference, image)

		# 	format = f"HEIC {quality}%"
		# 	print(f"{filename:20} format: {format:14}   bpp: {bpp:.2f}   PSNR↑: {psnr_value:.2f}    SSIM↑: {ssim_value:.4f}    FLIP↓: {flip_value:.4f}    LPIPS↓: {lpips_value:.4f}")

		# 	label = f"{quality}"
		# 	if quality != min(jpegQualityLevels) and quality != max(jpegQualityLevels):
		# 		label = ""
		# 	records.append(Record("HEIC", label, bpp, psnr_value, ssim_value, flip_value, lpips_value))


		os.makedirs(f"{args.output}/{filename}", exist_ok=True)

		#####################################
		### WRITE RECORDS TO FILE
		#####################################
		# for record in records:

		with open(f"{args.output}/{filename}/data.csv", "w", newline="") as f:
			writer = csv.writer(f, delimiter=";")
			writer.writerow(Record._fields)
			writer.writerows(records)



		
		#####################################
		### FIGURE 1 - PSNR
		#####################################

		# first plot of a reference image clears all previous plots
		plt.close("all")
		plt.clf()

		colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]

		plt.figure(1)
		plt.xlim( 0, 4.5)

		algorithms = list(dict.fromkeys([t.algorithm for t in records]))
		print(algorithms)

		# for algorithm in algorithms:
		for index, algorithm in enumerate(algorithms):
			entries = [t for t in records if t.algorithm == algorithm]
			bpps = [t.bpp for t in entries]
			psnrs = [t.psnr for t in entries]
			labels = [t.label for t in entries]

			plt.plot(bpps, psnrs, marker='o', linestyle='-', label=algorithm, clip_on=True, color=colors[index])
			
			# Also plot a line that reflects our size, comprising JPEG with 0.17 bit overhead
			if(algorithm == "JPEGLI"):
				bpps_ours = [bpp + 0.17 for bpp in bpps]
				plt.plot(bpps_ours, psnrs, linestyle='--', label="JPEGLI + 0.17 bit", clip_on=True, color=colors[index])

			indices = list(range(len(bpps)))
			# only print first and last label for variable-rate formats, to avoid too much noise in the plot
			if(algorithm != "ASTC" and algorithm != "BC"):
				indices = [0, len(bpps) - 1]

			for i in indices:
				plt.text(bpps[i] + 0.0, psnrs[i] + 0.0, labels[i], fontsize=8, clip_on=True)
			
		
		# Axis labels and title
		plt.xlabel("bits per pixel")
		# plt.ylabel("PSNR↑")
		# plt.title(f"Compression - {filename}")

		# Tick step size on Y axis
		#plt.yaxis.set_major_locator(MultipleLocator(2))
		plt.gca().yaxis.set_major_locator(MultipleLocator(2))


		plt.grid(True)
		plt.legend()
		
		plt.savefig(f"{args.output}/{filename}/plot_psnr.png", dpi=300, bbox_inches="tight", pad_inches=0.05)
		# plt.show()

		#####################################
		### FIGURE 2 - SSIM
		#####################################
		
		plt.figure(2)
		plt.xlim( 0, 4.5)

		minVal = 1000
		maxVal = 0

		# for algorithm in algorithms:
		for index, algorithm in enumerate(algorithms):
			entries = [t for t in records if t.algorithm == algorithm]
			bpps = [t.bpp for t in entries]
			ssims = [t.ssim for t in entries]
			labels = [t.label for t in entries]

			plt.plot(bpps, ssims, marker='o', linestyle='-', label=algorithm, clip_on=True, color=colors[index])

			# Also plot a line that reflects our size, comprising JPEG with 0.17 bit overhead
			if(algorithm == "JPEGLI"):
				bpps_ours = [bpp + 0.17 for bpp in bpps]
				plt.plot(bpps_ours, ssims, linestyle='--', label="JPEGLI + 0.17 bit", clip_on=True, color=colors[index])

			indices = list(range(len(bpps)))
			# only print first and last label for variable-rate formats, to avoid too much noise in the plot
			if(algorithm != "ASTC" and algorithm != "BC"):
				indices = [0, len(bpps) - 1]

			for value in ssims:
				minVal = min(minVal, value)
				maxVal = max(maxVal, value)

			for i in indices:
				plt.text(bpps[i] + 0.0, ssims[i] + 0.0, labels[i], fontsize=8, clip_on=True)
		
		# Axis labels and title
		plt.xlabel("bits per pixel")
		# plt.ylabel("SSIM↑")
		# plt.title(f"Compression - {filename}")

		stepsize = round((maxVal - minVal) / 8, 2)
		plt.gca().yaxis.set_major_locator(MultipleLocator(stepsize))
		plt.grid(True)
		plt.legend()
		
		plt.savefig(f"{outputDirectory}/{filename}/plot_ssim.png", dpi=300, bbox_inches="tight", pad_inches=0.05)
		# plt.show()

		#####################################
		### FIGURE 3 - FLIP
		#####################################
		
		plt.figure(3)
		plt.xlim( 0, 4.5)

		minVal = 1000
		maxVal = 0

		# for algorithm in algorithms:
		for index, algorithm in enumerate(algorithms):
			entries = [t for t in records if t.algorithm == algorithm]
			bpps = [t.bpp for t in entries]
			flips = [t.flip for t in entries]
			labels = [t.label for t in entries]

			plt.plot(bpps, flips, marker='o', linestyle='-', label=algorithm, clip_on=True, color=colors[index])

			# Also plot a line that reflects our size, comprising JPEG with 0.17 bit overhead
			if(algorithm == "JPEGLI"):
				bpps_ours = [bpp + 0.17 for bpp in bpps]
				plt.plot(bpps_ours, flips, linestyle='--', label="JPEGLI + 0.17 bit", clip_on=True, color=colors[index])

			indices = list(range(len(bpps)))
			# only print first and last label for variable-rate formats, to avoid too much noise in the plot
			if(algorithm != "ASTC" and algorithm != "BC"):
				indices = [0, len(bpps) - 1]

			for value in flips:
				minVal = min(minVal, value)
				maxVal = max(maxVal, value)

			for i in indices:
				plt.text(bpps[i] + 0.0, flips[i] + 0.0, labels[i], fontsize=8, clip_on=True)
		
		# Axis labels and title
		plt.xlabel("bits per pixel")
		# plt.ylabel("FLIP↓")
		# plt.title(f"Compression - {filename}")

		stepsize = round((maxVal - minVal) / 8, 2)
		# stepsize = round(round(100 * (maxVal - minVal) / 10) / 100, 2)
		plt.gca().yaxis.set_major_locator(MultipleLocator(stepsize))
		plt.grid(True)
		plt.legend()
		
		plt.savefig(f"{outputDirectory}/{filename}/plot_flip.png", dpi=300, bbox_inches="tight", pad_inches=0.05)

		#####################################
		### FIGURE 4 - LPIPS
		#####################################
		
		plt.figure(4)
		plt.xlim( 0, 4.5)

		minVal = 1000
		maxVal = 0

		for index, algorithm in enumerate(algorithms):
			entries = [t for t in records if t.algorithm == algorithm]
			bpps = [t.bpp for t in entries]
			lpipss = [t.lpips for t in entries]
			labels = [t.label for t in entries]

			plt.plot(bpps, lpipss, marker='o', linestyle='-', label=algorithm, clip_on=True, color=colors[index])

			# Also plot a line that reflects our size, comprising JPEG with 0.17 bit overhead
			if(algorithm == "JPEGLI"):
				bpps_ours = [bpp + 0.17 for bpp in bpps]
				plt.plot(bpps_ours, lpipss, linestyle='--', label="JPEGLI + 0.17 bit", clip_on=True, color=colors[index])

			indices = list(range(len(bpps)))
			# only print first and last label for variable-rate formats, to avoid too much noise in the plot
			if(algorithm != "ASTC" and algorithm != "BC"):
				indices = [0, len(bpps) - 1]

			for value in lpipss:
				minVal = min(minVal, value)
				maxVal = max(maxVal, value)

			for i in indices:
				plt.text(bpps[i] + 0.0, lpipss[i] + 0.0, labels[i], fontsize=8, clip_on=True)
		
		# Axis labels and title
		plt.xlabel("bits per pixel")
		# plt.ylabel("LPIPS↓")
		# plt.title(f"Compression - {filename}")

		stepsize = round((maxVal - minVal) / 8, 2)
		plt.gca().yaxis.set_major_locator(MultipleLocator(stepsize))
		plt.grid(True)
		plt.legend()
		
		plt.savefig(f"{outputDirectory}/{filename}/plot_lpips.png", dpi=300, bbox_inches="tight", pad_inches=0.05)

		



		# plt.show()
		
compareImages()

# # python compare_images.py -i E:/temp/jpeg_test/converted/bridge_1024_768.png -o "E:/temp/jpeg_test/eval"

 




